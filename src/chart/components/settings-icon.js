const toggleMenu = (d) => {
  ...
  const allMenus = d3
    .selectAll(".settings-menu-container")
...
const renderMenuOption = ({
  ...
  const hasUsers = (d) => d.itemMap.userIds.length > 0;

  const isAllowed = (d, title) => {
  ...
  const optionText = optionContainer
    ...
    .style("fill", "black")
    .style("font-size", 14)
    .style("opacity", (d) => (!isAllowed(d, title) ? 0.5 : 1))
    .text(title);
  ...
  optionContainer
    .on("mouseover", (d) => {
      if (d.isMenuOpen) {
        ...
        optionBackground.style(
          "cursor",
          !isAllowed(d, title) ? "not-allowed" : "default"
        );
        optionText.style(
          "cursor",
          !isAllowed(d, title) ? "not-allowed" : "default"
    ...
    .on("click", (datum) => {
      if (datum.isMenuOpen && isAllowed(datum, title)) {
        optionBackground.style("fill", config.backgroundColor);
...
module.exports = function iconLink({ svg, x = 5, y = 5, config }) {
  const isRoot = (d) => d.parentId === config.treeData.parentId;
  ...
  const settings = icon
    ...
    .style("visibility", (d) =>
      isRoot(d) && config.isViewOnly ? "hidden" : "visible"
    );
